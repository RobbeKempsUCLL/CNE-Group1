...
export async function httpTriggerDeleteBudget(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    try {
        ...
        const budget = await budgetService.deleteBudget(parseInt(id), userEmail);
        return {
            status: 200,
            jsonBody: budget
        };
...
app.http('httpTriggerDeleteBudget', {
    methods: ['DELETE'],
    authLevel: 'anonymous',
    handler: httpTriggerDeleteBudget
});